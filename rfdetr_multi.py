...
def test_all_photos(test_photos_dir: str = "test_photos", use_gpu=True):
    """
    Test RF-DETR on all images in the test_photos directory
    ...
    print("="*60)
    print("TESTING RF-DETR ON ALL TEST PHOTOS")
    print("="*60)
    ...
    for i, image_file in enumerate(image_files, 1):
        ...
        try:
            # Run RF-DETR with confusion matrix analysis
            results = rfdetr(image_path, label_path, use_gpu=use_gpu, create_overlay=True)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and ("--all" in sys.argv) and ("--no-gpu" in sys.argv):
        ...
        print("="*60)
        print("Testing RF-DETR with model_tests.py interface")
        print("="*60)
        ...
        print("Test Complete!")
        print("="*60)
        print("\nTo test all images, run: python rfdetr_multi --all")
        print("="*60)